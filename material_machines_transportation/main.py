...
from material_machines_transportation.column_weights import TEXT_COMPARISON_COLS, EQUALITY_COMPARISON_COLS, ATTACHMENTS_WEIGHT
...
from material_machines_transportation.config import PATH, FILTER_DATE, NORMALIZE_ARABIC_COLS, SIMILARITY_THRESHOLD, SIMILARITY_LEVELS, \
...
def main():
    columns = ['id', 'expenses_type', 'request_id', 'date_of_expenses',
               'transporter_name', 'operator_name', 'material_from_location', 'material_to_location']

    print('reading audit_request_lines...')

    df = pd.read_csv(PATH + '\\audit_request_line.csv', usecols=columns)
    df = df[df['expenses_type'] == 'material_machines_transportation']
